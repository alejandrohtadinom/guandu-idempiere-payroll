...
public class HR_BANABIH implements I_ReportExport {
	...
	public int exportToFile(MPrintFormat printFormat, MHRPaymentSelection hrPaySelection, StringBuffer err) {
		...
		// Payroll Process
		MHRProcess process = new MHRProcess(Env.getCtx(), hrPaySelection.getHR_Process_ID(), null);
		...
		try {
			...
			s_log.log(Level.WARNING, "Could not delete - " + m_file.getAbsolutePath(), e);
		}
		//	Number Format
		...
		try {
			...
			for (MHRPaymentSelectionLine pdl : details) {
				...
				if(m_Current_BPartner_ID != pdl.getC_BPartner_ID()) {
					...
					writeLine(pdl);
				} else if(m_CurrentMonth != null && m_CurrentMonth.equals(m_ProcessDateFormat.format(process.getDateAcct()))) {
	...
	/**
	 * Write Line
	 * @author <a href="mailto:tadinomalejandroh@gmail.com">Alejandro H Tadino M</a> 8/12/2014, 15:08:36
	 * @return void
	 * @throws IOException 
	 */
	private void writeLine(MHRPaymentSelectionLine m_Current_Pdl) throws IOException {
		//	Valid Null Value
		if(m_Current_Pdl == null)
			return;
		//	Process Business Partner
		String [] bpInfo = processBPartner(m_Current_Pdl.getC_BPartner_ID(), m_Current_Pdl.getAD_Org_ID(), m_Current_Pdl.get_TrxName());
		//	Line
		...
		line.append(bpInfo[BP_NATIONALITY])
			...
			.append(SEPARATOR)
			//	Amount
			.append(m_NumberFormat.format(m_CurrentAmt.doubleValue()).toString().replace(",", ".").replace(".", ""))
			.append(SEPARATOR)
			//	Employee Start Date
	...
	/**
	 * Process Business Partner
	 * @author <a href="mailto:tadinomalejandroh@gmail.com">Alejandro H Tadino M</a> 16/08/2014, 12:27:09
